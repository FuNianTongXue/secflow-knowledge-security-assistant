...
当问题包含年份并询问漏洞、CVE、高危漏洞或最新漏洞时，系统会先查询本地漏洞 RAG，再按年份调用 CVE 接口补充最新记录，最后将结果交给模型总结。非漏洞类问题不会强行走漏洞库检索，会将长期记忆、最近会话和相关历史上下文注入 LLM 后回答；如果 LLM 未配置或接口失败，则返回本地安全专家降级建议，并在结果里给出真实错误原因。
